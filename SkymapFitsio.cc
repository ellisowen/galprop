...
*/
	...
	int SkymapToFits (const Skymap<double> & skymap, const std::string & filename, const std::string & sUnit, const std::string & sType){
		...
		// new format is column per energy

	       string format="column"; //AWS20131230

               if(format=="vector")    //AWS20131230 for reference
	       {
		...
		delete[] array;
              ...
              if(format=="column")    //AWS20131230
	       {
		
		//Create the data table to store the actual image
	     

                char **tforms,**ttypes,**tunits;

                tforms = new char*[skymap.nSpectra()];
                ttypes = new char*[skymap.nSpectra()];
                tunits = new char*[skymap.nSpectra()];

                for (int icol=0;icol<skymap.nSpectra();icol++) 
		{
		
                 tforms[icol]=new char[15];
                 ttypes[icol]=new char[15];
                 tunits[icol]=new char[15];

		 strcpy (tforms[icol], "1D"       );
                 sprintf(ttypes[icol],"Bin %d",icol+1); //NB must be unique otherwise  columns have same content!
		 strcpy (tunits[icol], "Intensity");
		}


                int tfields= skymap.nSpectra(); // number of columns, convention as fits_create_tbl

		fits_create_tbl(fptr, BINARY_TBL, 0, tfields, ttypes, tforms, tunits, extname, &status);
		...
		//Convert the data to an array
		double *array;
                nelements=skymap.Npix();
		array = new double[nelements];

		//Write each energy to a  column
		
		// for(int ipix=0;ipix<nelements;ipix++){std::cout<<"ipix"<<ipix<<" ";std::cout<<skymap[ipix];std::cout<<std::endl;}

               for (int icol=0;icol<skymap.nSpectra();icol++) 
	       {
		 
                 for(int ipix=0;ipix<nelements;ipix++) array[ipix]=skymap[ipix][icol];

		
		 // for(int ipix=0;ipix<nelements;ipix++) std::cout<<"ipix"<<ipix<<" icol="<<icol<<" array="<<array[ipix]<<std::endl;

                 int colnum=icol+1; // column number starts at 1

		 fits_write_col(fptr, TDOUBLE, colnum, 1, 1, nelements, array, &status);
