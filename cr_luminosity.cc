...
CR density  gcr.cr_density is in c/4pi * n(p) [ cm s^-1 * sr^-1  * cm^-3 MeV^-1    ]   AWS20140416
primary source function is    in c/4pi        [ cm s^-1 * sr^-1  * cm^-3 MeV^-1 s-1]   AWS20140416
...
// identify primary electrons IMOS20070725
  int ielectrons=-1;
   ...
   if(ielectrons==-1) { WARNING("CR primary electrons not found!");  } //AWS20131106 remove return stat
   else {
      buf.str("");
      buf<<"  CR primary electrons found as species #"<<ielectrons;
      INFO(buf.str());
      //}                                                              //AWS20131106 moved down
...
// identify primary positrons AWS20131106
      ielectrons=-1;
   for(int i=0;i<n_species;i++) 
       if(!strcmp(gcr[i].name,"primary_positrons")) ielectrons=i;
   if(ielectrons==-1) { WARNING("CR primary positrons not found!");  } 
   else {
      buf.str("");
      buf<<"  CR primary positrons found as species #"<<ielectrons;
      INFO(buf.str());
...
//cout<<"cr_luminosity:  ip= "<<ip<<" CR_luminosity="<<  CR_luminosity<<endl;
   ...
   cout<<endl;
   cout<<"====================================================="<<endl;
   cout<<" CR primary positron luminosity="<< CR_luminosity <<" erg s^-1" <<endl;
     //<< CR_luminosity*gcr[ielectrons].normalization_factor<<endl;
   cout<<"====================================================="<<endl;
   cout<<endl;
...
// identify secondary positrons IMOS20070725
   ...
   if(ielectrons==-1) { WARNING("CR secondary positrons not found!");  } //AWS20131106 remove return stat
   else{
      buf.str("");
...
// identify secondary electrons AWS20100310
   ...
   if(ielectrons==-1) { WARNING("CR secondary electrons not found!");  }//AWS20131106 removed return stat
   else {
      buf.str("");
