...
int Galprop::create_galaxy() {//AWS20050816
  ...
  if (2 == galdef.n_spatial_dimensions) { // use average at y=0
	...
	if (galdef.synchrotron>=2) { //AWS20131003  (was ==2)
  ...
  if (3 == galdef.n_spatial_dimensions) {
      // ECC DEBUGGING 20141106
      /*cout << "HI:: nH_av(0,0,0)="<<nH_av(0, 5, 0,galaxy.dz, dzz, nHI) << endl;//IMOS20080114
      //cout << "HI:: nH_av_3D(0,0,0)=" << nH_av_3D(0,5,0,galaxy.dz, dzz, HICubeData, interp_HI_gas_cube)<<endl;//IMOS20080114
      cout << "H2:: nH_3D(0,0,0)=" << interp_CO_gas_cube(0,0,0,fX_CO(0),COCubeData) << endl;

      double H2AV=0, H23DAV=0; 

      for (int ix=-20; ix<=20 ; ix++){
        for (int iy=-20; iy<=20 ; iy++){
          for (int iz=-20; iz<=20 ; iz++){
            //cout << "HI Averaged="<<H2AV<<endl;             
            H23DAV += nH_av_3D(ix/10.,iy/10.,iz/10.,galaxy.dz, dzz, HICubeData, interp_HI_gas_cube) ;
            H2AV += nH_av(ix/10.,iy/10.,iz/10.,galaxy.dz, dzz, nHI) ;
          }
        }
      }

      cout << "HI Averaged="<<H2AV/(40.*40*40)<<endl; 
      cout << "HI3D Averaged="<<H23DAV/(40.*40*40)<<endl; 
      ...
      for (int ix=-20; ix<=20 ; ix++){
        for (int iy=-20; iy<=20 ; iy++){
          for (int iz=-20; iz<=20 ; iz++){
            //cout << "HI Averaged="<<H2AV<<endl;             
            H23DAV += nH_av_3D(ix/10.,iy/10.,iz/10.,galaxy.dz, dzz,fX_CO(0), COCubeData, interp_CO_gas_cube) ;
            H2AV += nH_av(ix/10.,iy/10.,iz/10.,galaxy.dz, dzz,fX_CO(0), nH2) ;
          }
        }
      }
      ...
      cout << "H2:: nH_av(0,0,0)="<<nH_av(0, 0, 0,galaxy.dz, dzz, fX_CO(0), nH2) << endl;//IMOS20080114
      cout << "H2:: nH_av_3D(0,0,0)=" << nH_av_3D(0,0,0,galaxy.dz, dzz,fX_CO(0), COCubeData, interp_CO_gas_cube) << endl;//IMOS20080114
      */
    ...
    if (galdef.nHI_model==1){
      ...
      galaxy.n_HI.d3[ix][iy][iz].s[0] =
        nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, HICubeData, interp_HI_gas_cube);  //ECC20141024
    }
    else if (galdef.nHI_model==3){
        galaxy.n_HI.d3[ix][iy][iz].s[0] =
                nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, nHI_3D);  //ECC20141024
    }
    ...
    if (galdef.nH2_model==1){
      ...
      galaxy.n_H2.d3[ix][iy][iz].s[0] =
        nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), COCubeData, interp_CO_gas_cube); //ECC20141024
    }
    else if (galdef.nH2_model==3){
        galaxy.n_H2.d3[ix][iy][iz].s[0] =
                nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), nH2_3D); //ECC20141024
    }
		/* ----------------------- END ECC20141024-------------------*/
	  ...
	  if ( galdef.synchrotron>=2)  //AWS20131003 (was ==2)
	  {
                                  int B_field_debug=0;                                                                                                                                 //AWS20140526
	    if(galdef.verbose==-1103) B_field_debug=1;                                                                                                                                 //AWS20140526
	    galaxy.B_field.d3[ix][iy][iz].s[0] = B_field_3D_model_tot(galdef.B_field_name, galdef.B_field_parameters, galaxy.x[ix], galaxy.y[iy], galaxy.z[iz],B_field_debug)/1.0e4;   //AWS20140526
