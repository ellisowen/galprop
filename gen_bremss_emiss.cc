...
#include "Bremsstrahlung.h" //AWS20120417
...
int Galprop::gen_bremss_emiss()  {
  ...
  // Zero the brem emissivity arrays

  galaxy.bremss_emiss = 0.;
  galaxy.bremss_ionized_emiss = 0.;


 if (galdef.bremss == 1) //AWS20120417
 {
...
#pragma omp parallel for schedule(dynamic) default(shared) private(cs_HII,cs_HI,cs_He,cs,y,ye,fe,Ne1,IZ1,key,i) firstprivate(Ekin)
  for (int iEgamma = 0; iEgamma < galaxy.n_E_gammagrid; ++iEgamma) {
    ...
    for (int ip = 0; ip < gcr[ielectrons].n_pgrid; ip++) {
      ...
      if (1 == galdef.integration_mode) { // ### old integration ###
	if (gcr[ielectrons].Etot[ip] <= 1.02*Mele || 
	    gcr[ielectrons].Ekin[ip] - galaxy.E_gamma[iEgamma] <= 0.) 
	  continue; //Gulli20070810
	...
	if (cs_HI <= 0.) 
	  continue; ///cs_HI==0.) continue;
         
      } else { // ### new integration - analytical ###
	    
	if (ip >= gcr[ielectrons].n_pgrid-1) 
	  continue; // don't consider the last point since already included  //Gulli20070821
	 
	if (gcr[ielectrons].Etot[ip+1] <= 1.02*Mele || 
	    gcr[ielectrons].Ekin[ip+1] - galaxy.E_gamma[iEgamma] <= 0.) 
	  continue; //Gulli20070810
	  
	for (i = 0;i < 3; ++i)  
	  cs[0][i] = cs[1][i];   // reassign old points
...
//cout<<" lower integration limit falls between the grid points "<<cs[0][1]<<" "<<cs[1][1]<<" e= "<<gcr[ielectrons].Ekin[ip]<<" g= "<<galaxy.E_gamma[iEgamma]<<endl;
      ...
      if (2 == gcr[ielectrons].n_spatial_dimensions)
	for (int ir=0; ir<gcr[ielectrons].n_rgrid-1; ir++)
	  for (int iz=1; iz<gcr[ielectrons].n_zgrid-1; iz++) {
	    ...
	    if (electrons.d2[ir][iz].s[ip] > 0 &&  
		electrons.d2[ir][iz].s[ip+1] > 0) { //AWS20100503
		  
	      
	      if (1 == key) {  // case: lower integration limit falls between the grid points
		...
		fe*= pow(Ekin,ye);                                                      // electron flux @ Ekin
	      } else {  // case: integration between the grid points
...
// derive y (= power-law index of the total expression)
	...
	if(iEgamma==galaxy.n_E_gammagrid/2)
      ...
      if (3 == gcr[ielectrons].n_spatial_dimensions)
 ...
 }// (galdef.bremss == 1) //AWS20120417



  ///////////// use C++ version of bremssstrahlung. AWS20120417, in development

   if(galdef.bremss==2)
   {
    Bremsstrahlung bremsstrahlung;
    valarray<double> bremsstrahlung_parameters;
    bremsstrahlung.init("test",bremsstrahlung_parameters,0); // name,parameters,debug

    valarray<double> Ee, Egamma;
    valarray<double> fluxe;

    Ee      .resize(gcr[ielectrons].n_pgrid);
    Egamma  .resize(galaxy.n_E_gammagrid);

    fluxe   .resize(gcr[ielectrons].n_pgrid);

    for(int ip     =0; ip<gcr[ielectrons].n_pgrid; ip++)         Ee  [ip]     = gcr[ielectrons].Ekin[ip];
    for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid;iEgamma++) Egamma[iEgamma]= galaxy         .E_gamma[iEgamma];
    int method;
    method=1;
    valarray<double> emissivity_bremss;

    emissivity_bremss.resize(galaxy.n_E_gammagrid);
    ...
    if(galaxy.n_spatial_dimensions==2)
    {
      ...
      for(int iz=0; iz<gcr[ielectrons].n_zgrid; iz++)
      {
       ...
       for(int ip=0; ip<gcr[ielectrons].n_pgrid; ip++) 
       {
        fluxe[ip] = electrons.d2[ir][iz].s[ip];    
       
       }

       IZ1=1; Ne1=1; // neutral H
       emissivity_bremss  = bremsstrahlung.bremss_emiss( Ee, Egamma, fluxe, IZ1,  Ne1, method );

       IZ1=2; Ne1=2;//  neutral He
       emissivity_bremss += bremsstrahlung.bremss_emiss( Ee, Egamma, fluxe, IZ1,  Ne1, method ) * galdef.He_H_ratio;

       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
         galaxy.bremss_emiss.d2[ir][iz].s[iEgamma] =  emissivity_bremss [iEgamma];


       IZ1=1; Ne1=0; // ionized H
       emissivity_bremss  = bremsstrahlung.bremss_emiss( Ee, Egamma, fluxe, IZ1,  Ne1, method );

       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
         galaxy.bremss_ionized_emiss.d2[ir][iz].s[iEgamma] =  emissivity_bremss [iEgamma];
    ...
    if(galaxy.n_spatial_dimensions==3)
    {
      ...
      for(int iz=0; iz<gcr[ielectrons].n_zgrid; iz++)
      {
       ...
       for(int ip=0; ip<gcr[ielectrons].n_pgrid; ip++) 
       {
        fluxe[ip] = electrons.d3[ix][iy][iz].s[ip];    
       ...
       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
         galaxy.bremss_emiss.d3[ix][iy][iz].s[iEgamma] =  emissivity_bremss [iEgamma];
       ...
       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
         galaxy.bremss_ionized_emiss.d3[ix][iy][iz].s[iEgamma] =  emissivity_bremss [iEgamma];
