...
#include "EnergyDispersion.h"   // AWS20131030 used by ProductionMatrices
#include "ProductionMatrices.h" // AWS20120413
...
// Helium: derive y (= power-law index of the total expression)
  ...
  ///////////////////////////////////////////////////////////
   ...
   if(galdef.pi0_decay>=4)                                        // AWS20121220
   {
    INFO("using ProductionMatrices hadronic model");              // AWS20121220
    

    ProductionMatrices pm;
    string hadronic_model="undefined";                                         // AWS20121220
    if(galdef.pi0_decay== 4)  hadronic_model= "Huang";                         // AWS20121220
    if(galdef.pi0_decay== 5)  hadronic_model= "Dermer";                        // AWS20121220
    if(galdef.pi0_decay== 6)  hadronic_model= "Kamae";                         // AWS20121220
    if(galdef.pi0_decay== 7)  hadronic_model= "Kachelriess";                   // AWS20121221 K&O recipe ppfrag reac=0: Kamae without diffractive below 50 GeV
    if(galdef.pi0_decay== 8)  hadronic_model= "Dermer_Kachelriess";            // AWS20121221 
    if(galdef.pi0_decay== 9)  hadronic_model= "Kamae_Kachelriess";             // AWS20121221          
    if(galdef.pi0_decay==10)  hadronic_model= "Kachelriess2013";               // AWS20130305 K&O 2013 QGSJET 4  with interpolation to Kamae
    if(galdef.pi0_decay==11)  hadronic_model= "Dermer_Kachelriess2013";        // AWS20131030 K&O 2013 QGSJET 4  with Dermer at low energies
    if(galdef.pi0_decay==12)  hadronic_model= "Dermer_Kachelriess2013_heavy";  // AWS20131030 K&O 2013 QGSJET 4  with Dermer at low energies, factor for A>2

    if(galdef.pi0_decay==20)  hadronic_model= "FLUKA";                  // AWS20130305 changed number

    INFO("using ProductionMatrices hadronic model:");             // AWS20121220
    INFO(hadronic_model);                                         // AWS20121220

    pm.init("kinetic_energy","MeV", 0, hadronic_model);           // AWS20121220
    pm.read(configure.fFITSDataDirectory);

    INFO("using ProductionMatrices hadronic model:");             // AWS20121220
    INFO(hadronic_model);                                         // AWS20121220
    INFO("NB for Kachelriess, Dermer_Kachelriess and Kamae_Kachelriess, ppfrag needs gamfrag.dat and apfrag.dat in run directory !");   // AWS20121220 since frag.f has names hard coded
    INFO("NB for Kachelriess2013 needs gam_HeHeR  gam_HepR  gam_pHeR  gam_ppR  in run directory !");   // AWS20130305 since init.f90  has names hard coded


    valarray<double> E_proton_rebin, E_gamma_rebin;
 
    E_proton_rebin .resize(gcr[iprotons].n_pgrid);
    E_gamma_rebin  .resize(galaxy       .n_E_gammagrid);


    for(int ip     =0; ip<gcr[iprotons].n_pgrid; ip++)         E_proton_rebin [ip]   = gcr[iprotons].Ekin[ip];
    for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid;iEgamma++) E_gamma_rebin[iEgamma]=galaxy.E_gamma[iEgamma];

    pm.rebin(E_proton_rebin, E_gamma_rebin);

    valarray<double> proton_spectrum, helium_spectrum;
    proton_spectrum.resize(gcr[iprotons].n_pgrid);
    helium_spectrum.resize(gcr[iprotons].n_pgrid);

    valarray<double> emissivity_huang;
    emissivity_huang.resize(galaxy.n_E_gammagrid);
    ...
    if(galaxy.n_spatial_dimensions==2)
    {
      ...
      for(int iz=0; iz<gcr[iprotons].n_zgrid; iz++)
      {
       ...
       for(int ip=0; ip<gcr[iprotons].n_pgrid; ip++) 
       {
        proton_spectrum[ip] = gcr[iprotons].cr_density.d2[ir][iz].s[ip];    
        helium_spectrum[ip] = gcr[iHelium ].cr_density.d2[ir][iz].s[ip]* gcr[iHelium ].A; 
       }

       emissivity_huang = pm.emissivity_total(proton_spectrum,helium_spectrum);

       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
	 galaxy.pi0_decay_emiss.d2[ir][iz].s[iEgamma] =  emissivity_huang [iEgamma];
                                                     
      } // ir iz

       //   pm.print();
     } // galaxy.n_spatial_dimensions==2
   ...
   if(galaxy.n_spatial_dimensions==3)
    {
      ...
      for(int iz=0; iz<gcr[iprotons].n_zgrid; iz++)
      {
       ...
       for(int ip=0; ip<gcr[iprotons].n_pgrid; ip++) 
       {
        proton_spectrum[ip] = gcr[iprotons].cr_density.d3[ix][iy][iz].s[ip];    
        helium_spectrum[ip] = gcr[iHelium ].cr_density.d3[ix][iy][iz].s[ip]* gcr[iHelium ].A; 
       }
    
       emissivity_huang = pm.emissivity_total(proton_spectrum,helium_spectrum);
      
       for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++)
        galaxy.pi0_decay_emiss.d3[ix][iy][iz].s[iEgamma] =   emissivity_huang [iEgamma];
                                                         
      } // ix iy iz

    } // galaxy.n_spatial_dimensions==3

   }//   if(galdef.pi0_decay==4)
