...
#pragma omp parallel for schedule(dynamic) default(shared)
	    ...
	    */
	    ...
	    //}
            ...
            // Added By ECC20150125.  Calculate the total column density of the gas surveys 
            // in order to renormalize the 3D gas model to the total survey column density
            // i.e. since 3D structure is already included in the gas model, we don't want to 
            // also impose the 3d structure gas map annuli which use a different deconvolution 
            // technique. 
           double w_HI_tot;
           double w_H2_tot;  // Scale factors for renormalizing analytical gas model to survey column densities.
           if (galdef.nHI_model == 2 || galdef.nH2_model==2){
                double CD_HI=0;
                double CD_H2=0;
                double Mod_HI=0;
                double Mod_H2=0;  // Scale factors for renormalizing analytical gas model to survey column densities.            
                for ( int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++ ){
                    ...
                    CD_HI += galaxy.hpHIR[co][i_Ring];
                    CD_H2 += 2*av_X_CO*galaxy.hpCOR[co][i_Ring]; 
                    Mod_HI+= n_HI_av[i_Ring]; 
                    Mod_H2+= n_H2_av[i_Ring];
                }
                // These are only used if galdef params: HI_model==2 || H2_model ==2
                w_HI_tot = CD_HI/Mod_HI;
                w_H2_tot = CD_H2/Mod_H2;  // Scale factors for renormalizing analytical gas model to survey column densities.
                // End ECC additions
            }

            for ( int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++ ) {
                
                double w_HI, w_H2;  // Scale factors for renormalizing analytical gas model to survey column densities.
                ...
                // ECC20150126 If the gas models are gas cubes, renormalize each pixel to the total column density instead of the annuli. 
                //cout << "healpix: w_HI, w_HI_tot:" << w_HI << ", " << w_HI_tot << endl;
                if (galdef.nHI_model == 2){
                    w_HI = w_HI_tot; 
                }
                if (galdef.nH2_model == 2){
                    w_H2 = w_H2_tot; 
                }

                if (galdef.renorm_off==1){
                    w_HI = 1; 
                    w_H2 = 2*av_X_CO; 
                }

                //ostringstream buf;
                //buf<<" w_HI_tot "<< w_HI_tot;
                //INFO(buf.str());
...
//			  emiss_HII[i_Ring][iEgamma] =0.; //IMOS20080114*
                    ...
                    // DEBUG added by ECC20150128.  Just outputs a single gas component to the full pion skymap.
                    // 0 for all components
                    // 1 for HI only
                    // 2 for H2 only
                    // 3 for HII only
                    if (galdef.single_component==1){
                        galaxy.pi0_decay_hp_skymap[co][iEgamma] += emiss_HI[i_Ring][iEgamma]*w_HI;
                    }
                    else if (galdef.single_component==2){
                        galaxy.pi0_decay_hp_skymap[co][iEgamma] += emiss_H2[i_Ring][iEgamma]*w_H2;
                    }
                    else if (galdef.single_component==3){
                        galaxy.pi0_decay_hp_skymap[co][iEgamma] += emiss_HII[i_Ring][iEgamma];
                    }
                    else{
                        // default 
            ...
            for ( int i_lat =0; i_lat <galaxy.n_lat; i_lat++ ) {
                ...
                if (galdef.nHI_model == 2 || galdef.nH2_model==2){
                    ...
                    for ( int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++ ){
                        ...
                        CD_HI += galaxy.HIR.d3[i_long][i_lat][i_Ring].s[0];
                        CD_H2 += 2.*av_X_CO*galaxy.COR.d3[i_long][i_lat][i_Ring].s[0];
                        Mod_HI+= n_HI_av[i_Ring]; 
                        Mod_H2+= n_H2_av[i_Ring];
                    }
                    // These are only used if galdef params: HI_model==2 || H2_model ==2
                ...
                for ( int i_Ring=0; i_Ring<galaxy.n_Ring; i_Ring++ ) {
                    ...
                    // ECC20150126 If the gas models are gas cubes, renormalize each pixel to the total column density instead of the annuli. 
                    //cout << "mapcube: w_HI, w_HI_tot:" << w_HI << ", " << w_HI_tot << endl;
                    if (galdef.nHI_model == 2){
                        w_HI = w_HI_tot; 
                    }
...
int Galprop::gen_pi0_decay_skymap_pixel ( const double l, const double b, vector<double> &n_H_av, vector<double> &n_HI_av, vector<double> &n_H2_av, vector<double> &n_CO_av, vector< vector<double> > &emiss_av, vector< vector<double> > &emiss_HII, vector< vector<double> > &emiss_HI, vector< vector<double> > &emiss_H2, const int n_Ring ) {
    ...
    while ( complete==0 ) {
        ...
        // nHI
        if (galdef.nHI_model == 1)
            nHI_av_lb = nH_av_lb(l, b, db, ddb, d, dd, ddd, nHI);
        else if (galdef.nHI_model == 2)
            //nHI_av_lb = nH_av_lb_3D(l, b, db, ddb, d, dd, ddd, HICubeData, interp_HI_gas_cube);
            nHI_av_lb = nH_av_lb_3D_rlb(l, b, db, ddb, d, dd, ddd, HICubeData_rlb, interp_HI_gas_cube_rlb);
        else if (galdef.nHI_model == 3)
            nHI_av_lb = nH_av_lb_3D(l, b, db, ddb, d, dd, ddd, nHI_3D);

        // nH2
        if (galdef.nH2_model == 1)
            nCO_av_lb = 2. * nH_av_lb(l, b, db, ddb, d, dd, ddd, 1.0, nH2);
        else if (galdef.nH2_model == 2)
            //nCO_av_lb = 2. * nH_av_lb_3D(l, b, db, ddb, d, dd, ddd, 1.0, COCubeData, interp_CO_gas_cube);
            nCO_av_lb = 2. * nH_av_lb_3D_rlb(l, b, db, ddb, d, dd, ddd, 1.0, COCubeData_rlb, interp_CO_gas_cube_rlb);
        
        else if (galdef.nH2_model == 3)
            nCO_av_lb = 2. * nH_av_lb_3D(l, b, db, ddb, d, dd, ddd, 1.0, nH2_3D);
            ...
            if ( lmin < ledge && ledge < lmax && i_Ring1 != i_Ring2 && galaxy.R_bins[galaxy.n_Ring+min ( i_Ring1,i_Ring2 ) ] < Rsun ) { //IMOS20080114**
                ...
                if ( i_Ring1 != i_Ring ) {
                    if (galdef.nHI_model==1)
                        ...
                        //nHI_av_lb1 =   nH_av_lb_3D ( lmin,b,db,ddb,d,dd,ddd,HICubeData, interp_HI_gas_cube);
                        nHI_av_lb1 = nH_av_lb_3D_rlb(lmin, b, db, ddb, d, dd, ddd, HICubeData_rlb, interp_HI_gas_cube_rlb);
                    else if (galdef.nHI_model == 3)
                        nHI_av_lb1 =   nH_av_lb_3D(lmin, b, db, ddb, d, dd, ddd, nHI_3D);


                    if (galdef.nH2_model==1)
                        nCO_av_lb1 =2.*nH_av_lb ( lmin,b,db,ddb,d,dd,ddd,1.0,nH2 );
                    else if (galdef.nH2_model==2)
                        //nCO_av_lb1 =2.*nH_av_lb_3D ( lmin,b,db,ddb,d,dd,ddd,1.0,COCubeData, interp_CO_gas_cube);
                        nCO_av_lb1 = 2. * nH_av_lb_3D_rlb(lmin, b, db, ddb, d, dd, ddd, 1.0, COCubeData_rlb, interp_CO_gas_cube_rlb);
                    else if (galdef.nH2_model == 3)
                        nCO_av_lb1 = 2. * nH_av_lb_3D(lmin, b, db, ddb, d, dd, ddd, 1.0, nH2_3D);
...
//	      nHII_av_lb1=   nH_av_lb(lmin,b,db,ddb,d,dd,ddd,nHII); //IMOS20080114*
                    ...
                    if (galdef.nHI_model==1)
                        ...
                        //nHI_av_lb1 =   nH_av_lb_3D ( lmax,b,db,ddb,d,dd,ddd,HICubeData, interp_HI_gas_cube );
                        nHI_av_lb1 = nH_av_lb_3D_rlb(lmax, b, db, ddb, d, dd, ddd, HICubeData_rlb, interp_HI_gas_cube_rlb);
                    else if (galdef.nHI_model == 3)
                        nHI_av_lb1 =   nH_av_lb_3D( lmax, b, db, ddb, d, dd, ddd, nHI_3D );

                    if (galdef.nH2_model==1)
                        nCO_av_lb1 =2.*nH_av_lb ( lmax,b,db,ddb,d,dd,ddd,1.0,nH2 );
                    else if (galdef.nH2_model==2)
                        //nCO_av_lb1 =2.*nH_av_lb_3D ( lmax,b,db,ddb,d,dd,ddd,1.0,COCubeData, interp_CO_gas_cube );
                        nCO_av_lb1 = 2. * nH_av_lb_3D_rlb(lmax, b, db, ddb, d, dd, ddd, 1.0, COCubeData_rlb, interp_CO_gas_cube_rlb);
                    else if (galdef.nH2_model == 3)
                        nCO_av_lb1 = 2. * nH_av_lb_3D( lmax, b, db, ddb, d, dd, ddd, 1.0, nH2_3D );
...
//		  for(int j=0;j<3;j++) {cout<<" x,f= "; for(int i=0;i<8;i++) cout<<x[i][j]<<" "<<f[i]<<"   "; cout<<endl;} exit(0);
            ...
            // ECC: Similar to above selectable debug, but specified in galdef. Sets CR density to 1 
            if (galdef.uniform_emiss==1)
            {
                delta = 1;
            }
