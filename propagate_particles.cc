...
int Galprop::propagate_particles() { //AWS20050816
  ...
  // spiral_fraction_particle = particle; 
  particle=gcr[0];
  particle.create_transport_arrays();
  // spiral_fraction_particle_init = 1; 
  ...
  for (int r_net_iter=network_iterations; r_net_iter>=1; --r_net_iter) { //IMOS20030129
    ...
    if(galdef.primary_positrons==1 && r_net_iter <= galdef.network_iter_compl && galdef.positron_norm_flux > 0) positrons_normalize();  //AWS20131023
