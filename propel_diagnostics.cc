...
//last_cr_density.print(); 
 ...
 for(ip=0;ip<particle.n_pgrid;ip++)
   diff_cr_density.d2[ir][iz].s[ip]=fabs(diff_cr_density.d2[ir][iz].s[ip]);//AWS20110121
 

 if(galdef.control_diagnostics==1){INFO("diff_cr_density");diff_cr_density.print();}
 }
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ir,ip) 
 ...
 for(ir=0;ir<particle.n_rgrid;ir++){            //AWS20110119
  if(ir>0)                                      //AWS20110119
  ...
  if(ir<particle.n_rgrid-1) //AWS20110119
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ir,ip) 
 ...
 for(iz=0;iz<particle.n_zgrid;iz++){      //AWS20110119
  if(iz>0)                                //AWS20110119
  ...
  if(iz<particle.n_zgrid-1)               //AWS20110119
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz,ir,ip) 
 for(ir=0;ir<particle.n_rgrid;ir++){
 for(iz=0;iz<particle.n_zgrid;iz++){
 for(ip=0;ip<particle.n_pgrid;ip++){      //AWS20110119
  if(ip>0)                                //AWS20110119
  ...
  if(ip<particle.n_pgrid-1)               //AWS20110119
...
total_source_function.print();  
 ...
 Distribution total_source_function_over_dt= total_source_function; //AWS20110131 source function now has dt like alpha (2D only so far)
 total_source_function_over_dt/=dt;
 dphidt+=   total_source_function_over_dt;   
...
*/
 ...
 if(galdef.control_diagnostics>=5)                   //AWS20110130
 { 
 ...
 double tolerance = 0.0;                              //AWS20110121  i.e. require machine accuracy
 int    converged = 0;                                //AWS20110114
 if (diff_cr_density.max()<= tolerance) converged = 1;//AWS20110114
...
buf.str("");
 ...
 buf<< " yrs";
 buf<< " converged="<<converged;//AWS20110114
...
total_source_function_over_dt.delete_array();

 return converged; //AWS20110114
}
...
//last_cr_density.print(); 
 ...
 for(iz=0;iz<particle.n_zgrid;iz++)
 for(ip=0;ip<particle.n_pgrid;ip++)
   diff_cr_density.d3[ix][iy][iz].s[ip]=fabs(diff_cr_density.d3[ix][iy][iz].s[ip]);//AWS20110121
...
total_source_function.print();  
 ...
 //dphidt+=   total_source_function;                                //AWS20110201
      
 Distribution total_source_function_over_dt= total_source_function; //AWS20110201 source function now has dt like alpha 
 total_source_function_over_dt/=dt;
 dphidt+=   total_source_function_over_dt;   
