...
extern "C" double     gsl_sf_gamma_inc       (const double x, const double z);
...
double Galprop::source_distribution (const double x, 
  ...
  if (1 == srcModel)  //arbitrary parametrization
    {         
      zscale=parameters[0]; //AWS20131108
      alpha= parameters[1];
      beta = parameters[2]; 
      rmax = parameters[3]; 
      ...
      ro=8.5;
//    zscale=0.200;  //nominal value AWS20131108 replaced by parameters[0]
      double x      = r      + roff;
      double xo     = ro     + roff;
      double xconst = rconst + roff;
      
      if(zscale==0.) //AWS20131108 default=0.2 but in some galdef files set to 0.
      {
	cout<<"source_distribution: zscale=0 invalid! set source_parameters_0 to a value >0 and rerun. Exiting!"<<endl;
        exit(1);
      }
...
//   zscale=0.200;  //nominal value AWS20131108 replaced by parameters[0]
  ...
  if (20 == srcModel)  //SNR:  Case and Bhattacharya 3rd Compton p437
  {
    alpha=.5;
    beta=1.;
    ro=8.5;
