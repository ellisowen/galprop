...
int Galprop::store_pi0_decay_HIR_skymap() {
  ...
  if (3 == galdef.skymap_format) {
    ...
    for (int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++){
      ...
      filename +=".gz";
      SkymapToFits(galaxy.pi0_decay_HIR_hp_skymap[i_Ring], filename, "Energy", "MeV");
    }
    ...
    if(galdef.skymap_format == 1 || galdef.skymap_format == 2){ //!< Mapcube output compatable with Glast science tools
      ...
      status = store_mapcube_skymap(&array[0], galaxy.E_gamma, galaxy.pi0_decay_HIR_skymap.n_zgrid, galaxy.n_E_gammagrid, "pion_decay_HIR_mapcube_", true);
    }
