...
int Galprop::store_synch_skymap() { //AWS20050817
  ...
  if (3 == galdef.skymap_format) {
    ...
    filename =       configure.fOutputDirectory + configure.fOutputPrefix + "synchrotron_P_healpix_" + galdef.galdef_ID + ".gz";                    //AWS20131004
    SkymapToFits(galaxy.synchrotron_P_hp_skymap, filename, "Frequency", "Hz");                                                                      //AWS20131004

    filename =       configure.fOutputDirectory + configure.fOutputPrefix + "synchrotron_polang_healpix_" + galdef.galdef_ID + ".gz";               //AWS20131004
    SkymapToFits(galaxy.synchrotron_polang_hp_skymap, filename, "Frequency", "Hz");                                                                 //AWS20131004
	...
	(2 == galdef.skymap_format)) { //!< Mapcube output compatible with Glast science tools
