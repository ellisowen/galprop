...
*/
  ...
  // this is a stand-alone routine so define the constants 
   ...
   const double c   = 2.99792e10;     // speed of light, cm s^-1
   const double m_e =  9.10939e-28 ;  // electron mass, g
   const double e_e =  4.80321e-10 ;  // electron charge, esu
   
   // derived constants

   const double emc     =      e_e   / ( m_e     * c   );     //  e  /mc
   const double e3mc2   =  pow(e_e,3) /( m_e * pow(c,2));     //  e^3/mc^2
  ...
  const double nu_c = 3./(4.*pi) * emc  * Bperp * gamma * gamma ; // critical frequency
  ...
  const double constant_reg=sqrt(3.)/2. *  e3mc2 * Bperp ;
  ...
  const double nu_c = 3./(2.*pi) * emc  * Brand * gamma * gamma ; // critical frequency
  const double x=nu/nu_c;
  ...
  const double  constant_random=2.*sqrt(3.) *  e3mc2 * Brand ;
