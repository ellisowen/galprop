...

{
 ...
 cout<<"synchrotron_emissivity_B_field: (x, y, z) = ("<<x<<", "<<y<<", "<<z<<")  ";
                ...
                double &I, double&Q, double &U, int debug=0)
{
  ...
  // computes Stokes parameters. The coordinate system is Galactic, with chi measured from Galactic N-S latitude meridians
  ...
  double chi;               //AWS20110309 for compatibility with Waelkens etal.

  chi  = atan2(Byp,Bzp);    // atan2(sin,cos) so that this is the angle between Z and projection of B on YZ plane
  //  chi  = -chi;              //AWS20110908  for compatibility with WMAP, comparing U plots sign AWS20121023 removed, not required
  chi += acos(-1.0)/2. ;    // add pi/2 to get angle between Z and polarization direction which is perpendicular to   projection of B on YZ plane +or- same since use 2*chi for Stokes  AWS20110309
  ...
  // as in Hammurabi, Waelkens et al. (2009) A&A 495,697 
  I =        perp+par;                   //AWS20110308
  Q =       (perp-par)*cos(2.*chi);
  U =       (perp-par)*sin(2.*chi);
  ...
  if(debug==1||debug==2)
  {
  cout << "stokes: x = "<<x    <<" y= "<<y <<" z= "<<z  <<endl;
  cout <<        " x0= "<<x0   <<" y0="<<y0<<" z0="<<z0 <<endl;
  cout<<" Bx  ="<<Bx <<" By  ="<<By <<" Bz  ="<<Bz      <<endl;
  cout<<" perp="<<perp<<" par="<<par                    <<endl;
  cout << "  /  l = "<<l/dtr<<" b= "<<b/dtr             <<endl;
  cout<<" Stokes I = "<<I<<" Q = "<<Q  <<" U = "<<U     <<endl;     
  cout<<" chi (deg) = "<<chi/dtr                        <<endl;
  cout<<" total intensity = "<<perp+par                 <<endl;
  cout<<"   pol intensity = "<<perp-par                 <<endl;          //AWS20110308
  cout<<" unpol intensity = "<<2*par                    <<endl;
  cout<<" polarization="<<(perp-par)/(perp+par)         <<endl;
  }
